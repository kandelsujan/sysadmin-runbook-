...
# runs. It does, however, stay visible — INFO on success, WARN plus an
# error-detail file if find hits trouble, so a silent partial prune can't
# happen.
#
...
#                 containing only empty subdirs) collapse in one pass.
# Directories still holding files from other years are left untouched.
prune_empty_source_dirs() {
        local prune_err
        if prune_err=$(find "${SOURCE_SHARE}" -mindepth 1 -type d -empty -delete 2>&1); then
                log "INFO" "Pruned empty directories under ${SOURCE_SHARE}"
        else
                log "WARN" "Empty-directory prune had errors; some empty dirs may remain under ${SOURCE_SHARE}. Archive itself succeeded."
                if [[ -n "${prune_err}" ]]; then
                        printf '%s\n' "${prune_err}" >"${LOG_DIRECTORY}/PRUNE_ERRORS_${ARCHIVE_YEAR}"
                        log "WARN" "Prune error detail written to ${LOG_DIRECTORY}/PRUNE_ERRORS_${ARCHIVE_YEAR}"
                fi
        fi
